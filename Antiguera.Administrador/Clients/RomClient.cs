...
{
    public class RomClient : ClientBase<RomModel, RomTableModel>, IRomClient
    {
        public async override Task<RomTableModel> ListarTabela(string url)
        {
            var table = new RomTableModel();
            ...
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    {
                        var acessos = await response.Content.ReadAsAsync<ICollection<RomModel>>();

                        foreach (var acesso in acessos)
                        {
                            table.data.Add(new RomListTableModel()
                            {
                                Id = acesso.Id,
                                Nome = acesso.Nome,
