...
namespace AntigueraWebApi.Controllers
{
    ...
    public class AdminController : ApiController
    {
        ...
        public async Task<HttpResponseMessage> AtualizarAdmin([FromBody]UsuarioModel usuarioModel)
        {
            ...
            try
            {
                if(ModelState.IsValid)
                {
                    ...
                    if (acesso != null)
                    {
                        ...
                        {
                            logger.Error("AtualizarAdmin - Usuário não localizado");
                            stats.Status = HttpStatusCode.NotFound;
                            stats.Mensagem = "Usuário não localizado!";
                    ...
                    {
                        logger.Error("AtualizarAdmin - Id de acesso não localizado");
                        stats.Status = HttpStatusCode.NotFound;
                        stats.Mensagem = "Id de acesso não localizado!";
        ...
        public async Task<HttpResponseMessage> AtualizarSenhaAdmin([FromBody]UsuarioModel usuarioModel)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    {
                        logger.Error("AtualizarSenhaAdmin - Usuário não localizado");
                        stats.Status = HttpStatusCode.NotFound;
                        stats.Mensagem = "Usuário não localizado!";
        ...
        public async Task<HttpResponseMessage> ExcluirUsuario([FromBody] UsuarioModel usuarioModel)
        {
            ...
            try
            {
                if(ModelState.IsValid)
                {
                    ...
                    {
                        logger.Error("ExcluirUsuario - Usuário não localizado");
                        stats.Status = HttpStatusCode.NotFound;
                        stats.Mensagem = "Usuário não localizado!";
        ...
        public async Task<HttpResponseMessage> ExcluirAcesso([FromBody] AcessoModel acessoModel)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (role != null)
                    {
                        await appRole.DeleteAsync(role);
                    }
                    else
                    {
                        logger.Error("ExcluirAcesso - Id de acesso não localizado");
                        stats.Status = HttpStatusCode.NotFound;
                        stats.Mensagem = "Id de acesso não localizado!";
